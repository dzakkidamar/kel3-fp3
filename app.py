...
if page == "Edit Data (password is required)":
    ...
    if st.button("Sign in"):
        if password == correct_password:
            ...
            incorrect_password = True  # Set the flag to True if the password is incorrect

    # If signed in, hide the password input and provide feedback
    if signed_in:
        lock_edit_data = st.button("Lock Edit Data")
        if lock_edit_data:
            # Additional logic to perform when the user locks the edit data feature
            signed_in = False  # Reset the signed_in flag
            st.success("Edit Data feature locked successfully!")
    else:
        if incorrect_password:
            st.warning("Incorrect password. Please enter the correct password.")
        else:
            st.info("Please sign in to access the data.")
